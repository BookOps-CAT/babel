...
class CartSummary:
    ...
    def __init__(self, parent, **app_data):
        ...
        self.resultsTxt.grid(row=0, column=1, rowspan=5, sticky="snew", padx=10)
    ...
    def tabulate(self):
        ...
        self.resultsTxt.insert("1.0", "Cart summary:\n\n")
...
class ApplyGridsWidget:
    ...
    def __init__(self, parent, system_id, profile_id, distributions, **app_data):
        ...
        frm = Frame(self.top)
        frm.grid(row=0, column=0, sticky="snew", padx=10, pady=20)
        ...
        self.resourcesTrv = Treeview(
            frm, columns=columns, displaycolumns=columns[1:], show="headings", height=40
        )
        ...
        self.resourcesTrv.grid(row=0, column=0, rowspan=20, sticky="snew")

        scrollbar = Scrollbar(frm, orient="vertical", command=self.resourcesTrv.yview)
        ...
        btnFrm.grid(row=0, column=1, rowspan=20, sticky="snew", pady=20)
        btnFrm.rowconfigure(0, minsize=45)

        applyBtn = Button(btnFrm, image=applyImg, command=self.apply_distribution)
        applyBtn.grid(row=1, column=0, sticky="sw", padx=5, pady=5)
        self._createToolTip(applyBtn, "apply grid")

        removeBtn = Button(btnFrm, image=removeImg, command=self.remove_distribution)
        removeBtn.grid(row=2, column=0, sticky="sw", padx=5, pady=5)
        self._createToolTip(removeBtn, "remove grid")

        # grids frm
        gridFrm = Frame(self.top)
        gridFrm.grid(row=0, column=2, rowspan=20, sticky="snew", padx=5, pady=20)
        ...
        self.distrCbx.grid(row=1, column=0, sticky="snew", padx=5, pady=5)

        self.gridCbx = Combobox(
            gridFrm, state="readonly", font=RFONT, textvariable=self.selected_grid
        )
        self.gridCbx.grid(row=2, column=0, sticky="snew", padx=5, pady=5)

        Checkbutton(gridFrm, text="add to existing", variable=self.append_grid).grid(
            row=3, column=0, sticky="snw", padx=5, pady=5
        )

        Label(gridFrm, textvariable=self.total_qty, font=RFONT).grid(
            row=5, column=0, sticky="snw", padx=5, pady=5
        )

        Label(gridFrm, textvariable=self.total_amount, font=RFONT).grid(
            row=6, column=0, sticky="snw", padx=5, pady=5
        )

        self.create_resources_list()

        closeBtn = Button(gridFrm, text="close", width=10, command=self.top.destroy)
        closeBtn.grid(row=8, column=0, sticky="snw", padx=20, pady=5)

    def apply_distribution(self):
        # verify distribution and grid is selected
        if not self.selected_distr.get() or not self.selected_grid.get():
            messagebox.showwarning(
                "Missing data", "Please select distribution.", parent=self.top
            )
        else:
            # determine datastore id of orders to be modified
            order_ids = self.map_selection_to_order_ids()
            grid_id = get_id_from_index(self.selected_grid.get(), self.grid_idx)
            ...
            if order_ids:
                ...
                apply_grid_to_selected_orders(order_ids, grid_id, append=append)
                self.create_resources_list()
    ...
    def distr_observer(self, *args):
        if self.selected_distr.get():
            ...
            mlogger.debug("User seclected DistSet.did={dist_rec.did}")

            self.grid_idx = create_name_index(DistGrid, distset_id=dist_rec.did)
    ...
    def _treeview_sort_column(self, tv, col, reverse):
        ...
        tv.heading(
            col, command=lambda: self._treeview_sort_column(tv, col, not reverse)
        )
...
class SearchCartWidget:
    ...
    def __init__(self, parent, **app_data):
        ...
        self.keywordEnt.grid(row=0, column=0, sticky="snew", padx=2, pady=5)
        ...
        self.keytypeCbx.grid(row=0, column=1, sticky="snew", padx=2, pady=5)
        self.keytypeCbx.set(self.key_types[0])

        self.search_types = ["phrase", "keyword"]

        self.searchtypeCbx = Combobox(
            inputFrm, font=RFONT, values=self.search_types, state="readonly", width=10
        )
        ...
        self.searchBtn.grid(row=0, column=1, sticky="snw", padx=5, pady=5)

        self.closeBtn = Button(
            btnFrm, text="close", command=self.search_top.destroy, width=10
        )
        self.closeBtn.grid(row=0, column=2, sticky="snw", padx=5, pady=5)
        ...
        self.resultsTrv.grid(row=0, column=0, sticky="snew")
        self.resultsTrv.bind("<Double-Button-1>", self.select_item)

        scrollbar = Scrollbar(resFrm, orient="vertical", command=self.resultsTrv.yview)
...
class CartView(Frame):
    ...
    def __init__(self, parent, controller, **app_data):
        ...
        self.actionFrm.grid(row=0, column=0, sticky="snew", padx=5, pady=10)

        # register comboboxes and entries validation
        self.vlen = (self.register(self._onValidateName), "%i", "%W")
        self.vlqt = (self.register(self._onValidateQty), "%i", "%d", "%P")
        self.vlam = (self.register(self._onValidatePrice), "%i", "%d", "%P")
        self.vlds = (self.register(self._onValidateDiscount), "%i", "%d", "%P")

        self.addBtn = Button(self.actionFrm, image=addImg, command=self.add_resource)
        self.addBtn.grid(row=0, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.addBtn, "add title to cart")

        self.editBtn = Button(self.actionFrm, image=editImg, command=self.rename_cart)
        self.editBtn.grid(row=1, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.editBtn, "edit cart name")

        self.saveBtn = Button(
            self.actionFrm, image=self.saveImg, command=self.save_cart
        )
        self.saveBtn.grid(row=2, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.saveBtn, "save cart")

        self.gridBtn = Button(
            self.actionFrm, image=gridImg, command=self.selective_grids_widget
        )
        self.gridBtn.grid(row=3, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.gridBtn, "apply distribution to\nselected titles")

        self.fundBtn = Button(
            self.actionFrm, image=fundImgM, command=self.show_fund_widget
        )
        self.fundBtn.grid(row=4, column=0, sticky="sw", padx=10, pady=5)
        ...
        self.copyBtn = Button(self.actionFrm, image=copyImg, command=self.copy_cart)
        self.copyBtn.grid(row=6, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.copyBtn, "copy entire cart")

        self.searchBtn = Button(
            self.actionFrm, image=searchImg, command=self.search_widget
        )
        self.searchBtn.grid(row=7, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.searchBtn, "search cart")

        self.sierraBtn = Button(
            self.actionFrm, image=sierraImg, command=self.find_duplicates_widget
        )
        self.sierraBtn.grid(row=8, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.sierraBtn, "search Sierra")

        self.tabulateBtn = Button(
            self.actionFrm, image=calcImgM, command=self.tabulate_cart_widget
        )
        self.tabulateBtn.grid(row=9, column=0, sticky="sw", padx=10, pady=5)
        ...
        self.deleteBtn = Button(
            self.actionFrm, image=self.deleteImg, command=self.delete_cart
        )
        self.deleteBtn.grid(row=11, column=0, sticky="sw", padx=10, pady=5)
        self._createToolTip(self.deleteBtn, "delete cart")

        self.globdataFrm = Frame(self, relief="groove")
        self.globdataFrm.grid(row=0, column=1, sticky="snew", padx=20, pady=10)
        ...
        self.cartEnt.grid(row=1, column=0, columnspan=3, sticky="snew", padx=6, pady=2)

        Label(self.globdataFrm, text="library:").grid(
            row=2, column=0, sticky="snw", padx=5, pady=2
        )
        self.libCbx = Combobox(
            self.globdataFrm, textvariable=self.library, font=RFONT, state="readonly"
        )
        self.libCbx.grid(row=2, column=1, columnspan=2, sticky="snew", padx=5, pady=2)

        Label(self.globdataFrm, text="status:").grid(
            row=3, column=0, sticky="snw", padx=5, pady=2
        )
        self.statusCbx = Combobox(
            self.globdataFrm, font=RFONT, textvariable=self.status, state="readonly"
        )
        self.statusCbx.grid(
        ...
        self.distCbx = Combobox(
            self.globdataFrm, font=RFONT, textvariable=self.dist_set, state="readonly"
        )
        self.distCbx.grid(row=6, column=0, columnspan=3, sticky="snew", padx=5, pady=2)
        ...
        self.langCbx = Combobox(self.globdataFrm, font=RFONT, textvariable=self.lang)
        self.langCbx.grid(row=9, column=1, columnspan=2, sticky="snew", padx=5, pady=2)
        ...
        self.vendorCbx = Combobox(
            self.globdataFrm, font=RFONT, textvariable=self.vendor
        )
        self.vendorCbx.grid(
        ...
        self.mattypeCbx = Combobox(
            self.globdataFrm, font=RFONT, textvariable=self.mattype
        )
        ...
        self.audnCbx = Combobox(self.globdataFrm, font=RFONT, textvariable=self.audn)
        self.audnCbx.grid(row=12, column=1, columnspan=2, sticky="snew", padx=5, pady=2)
        self.audnCbx["state"] = "readonly"

        Label(self.globdataFrm, text="PO:").grid(
            row=13, column=0, sticky="snw", padx=5, pady=2
        )
        self.poEnt = Entry(self.globdataFrm, font=RFONT, textvariable=self.poperline)
        self.poEnt.grid(row=13, column=1, sticky="snew", padx=5, pady=2)
        self.poChb = Checkbutton(self.globdataFrm, variable=self.poChb_var)
        self.poChb.grid(row=13, column=2, sticky="sne", padx=5, pady=2)

        Label(self.globdataFrm, text="ord. note:").grid(
            row=14, column=0, sticky="snw", padx=5, pady=2
        )
        self.noteEnt = Entry(self.globdataFrm, font=RFONT, textvariable=self.note)
        self.noteEnt.grid(row=14, column=1, sticky="snew", padx=5, pady=2)
        self.noteChb = Checkbutton(self.globdataFrm, variable=self.noteChb_var)
        self.noteChb.grid(row=14, column=2, sticky="sne", padx=5, pady=2)
        ...
        self.priceEnt.grid(row=15, column=1, sticky="snew", padx=5, pady=2)
        self.priceChb = Checkbutton(self.globdataFrm, variable=self.priceChb_var)
        self.priceChb.grid(row=15, column=2, sticky="sne", padx=5, pady=2)
        ...
        self.discEnt = Entry(
            ...
            validate="key",
            validatecommand=self.vlds,
        )
        self.discEnt.grid(row=16, column=1, sticky="snew", padx=5, pady=2)
        self.discountChb = Checkbutton(self.globdataFrm, variable=self.discountChb_var)
        self.discountChb.grid(row=16, column=2, sticky="sne", padx=5, pady=2)

        self.applyBtn = Button(
            self.globdataFrm, text="apply", width=10, command=self.apply_globals
        )
        self.applyBtn.grid(row=17, column=1, sticky="snw", pady=10)

        self.fundLbl = Label(
            self.globdataFrm, textvariable=self.funds_tally, anchor=CENTER
        )
        self.fundLbl.grid(row=18, column=0, columnspan=3, sticky="snew", padx=5, pady=5)

        self.dispLbl = Label(
            self.globdataFrm, textvariable=self.orders_displayed, anchor=CENTER
        )
        self.dispLbl.grid(row=19, column=0, columnspan=3, sticky="snew", padx=5, pady=5)

        self.navFrm = Frame(self.globdataFrm)
        self.navFrm.grid(row=20, column=1, sticky="snew", padx=10, pady=10)

        self.startBtn = Button(self.navFrm, image=startImg, command=self.nav_start)
        self.startBtn.image = startImg
        self.startBtn.grid(row=0, column=0, sticky="sw", padx=2, pady=5)

        self.previousBtn = Button(
            self.navFrm, image=previousImg, command=self.nav_previous
        )
        self.previousBtn.image = previousImg
        self.previousBtn.grid(row=0, column=1, sticky="sw", padx=2, pady=5)

        self.nextBtn = Button(self.navFrm, image=nextImg, command=self.nav_next)
        self.nextBtn.image = nextImg
        self.nextBtn.grid(row=0, column=2, sticky="sw", padx=2, pady=5)

        self.endBtn = Button(self.navFrm, image=endImg, command=self.nav_end)
        self.endBtn.image = endImg
        self.endBtn.grid(row=0, column=3, sticky="sw", padx=2, pady=5)

        # individual order data
        self.ordsFrm = LabelFrame(self.globdataFrm, text="orders")
        self.ordsFrm.grid(row=0, column=3, rowspan=40, sticky="snew", padx=10, pady=5)

        self.xscrollbar = Scrollbar(self.ordsFrm, orient=HORIZONTAL)
        self.xscrollbar.grid(row=0, column=1, columnspan=7, sticky="nwe")
        self.yscrollbar = Scrollbar(self.ordsFrm, orient=VERTICAL)
        self.yscrollbar.grid(row=0, column=0, rowspan=41, sticky="nse", pady=5)
        ...
        self.preview_base.grid(row=1, column=1, rowspan=40, columnspan=7, sticky="nwe")
    ...
    def add_location(self, parent):
        ...
        loc = self.create_grid(self.tracker[ntb_id]["grid"]["locsFrm"], row)
        locs.append(loc)
    ...
    def apply_funds(self, listbox, selected):
        ...
        try:
            ...
            messagebox.showerror(
                "Funds error", f"Unable to apply funds correctly. Error: {e}"
            )
        else:
    ...
    def apply_grid_template(self, ntb_id):
        ...
        if grid_template_name != "":
            ...
            grid_record = get_record(
                DistGrid, distset_id=self.dist_id.get(), name=grid_template_name
            )
            r = 0
            locs = []
            for l in grid_record.gridlocations:
                ...
                qty = l.qty
                loc = self.create_grid(locsFrm, r, (None, branch, shelf, qty, ""))
                locs.append(loc)
                r += 1
            self.tracker[ntb_id]["grid"]["locs"] = locs
    ...
    def copy_cart(self):
        ccw = CopyCartWidget(
            self, self.cart_id.get(), self.cart_name.get(), **self.app_data
        )
    ...
    def copy_grid_to_template(self, ntb_id):
        ...
        Label(frm, text="new name:").grid(row=0, column=2, sticky="snew", padx=5)

        Label(frm, text="distribution:").grid(
            row=1, column=0, sticky="snw", padx=5, pady=5
        )
        distCbx = Combobox(
            frm, font=RFONT, textvariable=self.dist_template, state="readonly"
        )
        distCbx.grid(row=1, column=1, sticky="snew", padx=5, pady=5)
        ...
        self.newdistEnt = Entry(
            frm, font=RFONT, textvariable=self.new_dist, state="disabled"
        )
        self.newdistEnt.grid(row=1, column=2, sticky="snew", padx=5, pady=5)

        Label(frm, text="grid:").grid(row=2, column=0, sticky="snw", padx=5, pady=5)
        self.gridCbx = Combobox(
            frm, font=RFONT, textvariable=self.grid_template, state="readonly"
        )
        self.gridCbx.grid(row=2, column=1, sticky="snew", padx=5, pady=5)
        values = sorted(self.grid_idx.values())
        values.insert(0, "new grid")
        self.gridCbx["values"] = values
        self.newgridEnt = Entry(
            frm, font=RFONT, textvariable=self.new_grid, state="disabled"
        )
        self.newgridEnt.grid(row=2, column=2, sticky="snew", padx=5, pady=5)

        btnFrm = Frame(frm)
        btnFrm.grid(row=3, column=0, columnspan=2, sticky="snew", padx=10, pady=10)
        btnFrm.columnconfigure(0, minsize=200)

        okBtn = Button(
            btnFrm, image=self.saveImgS, command=lambda: self.save_new_template(ntb_id)
        )
        okBtn.grid(row=0, column=1, sticky="snw", padx=5, pady=10)
        cancelBtn = Button(btnFrm, image=self.deleteImgS, command=self.gridTop.destroy)
        cancelBtn.grid(row=0, column=2, sticky="snw", padx=5, pady=10)

    def create_add_locationBtn(self, parent):
        # recreate in new row
        add_locationBtn = Button(
            parent, image=self.addImgS, command=lambda: self.add_location(parent)
        )
        add_locationBtn.image = self.addImgS
        add_locationBtn.grid(row=3, column=0, sticky="nw", padx=5, pady=2)
    ...
    def create_grid(self, parent, row, loc=(None, "", "", "", "")):
        ...
        branchCbx.grid(row=0, column=1, sticky="snew", padx=2, pady=4)
        branchCbx.set(loc[1])
        ...
        shelfCbx.grid(row=0, column=2, sticky="snew", padx=2, pady=4)
        shelfCbx.set(loc[2])

        qtyEnt = Entry(
            unitFrm, font=RFONT, width=3, validate="key", validatecommand=self.vlqt
        )
        qtyEnt.grid(row=0, column=3, sticky="snew", padx=2, pady=4)
        ...
        fundCbx.grid(row=0, column=4, columnspan=2, sticky="snew", padx=2, pady=4)
        fundCbx.set(loc[4])
    ...
    def create_order_frame(self, parent, order):
        ...
        orderFrm.grid(row=1, column=0, sticky="snew", padx=5, pady=5)

        Label(orderFrm, text="lang:").grid(
            row=0, column=0, sticky="snw", padx=2, pady=2
        )
        langCbx = Combobox(
            orderFrm, values=sorted(self.lang_idx.values()), state="readonly"
        )
        langCbx.grid(row=0, column=1, sticky="snew", padx=5, pady=2)
        if order.lang_id:
            langCbx.set(self.lang_idx[order.lang_id])

        Label(orderFrm, text="ven:").grid(row=1, column=0, sticky="snw", padx=2, pady=2)
        vendorCbx = Combobox(
            orderFrm, values=sorted(self.vendor_idx.values()), state="readonly"
        )
        vendorCbx.grid(row=1, column=1, sticky="snew", padx=5, pady=2)
        if order.vendor_id:
        ...
        audnCbx = Combobox(
            orderFrm, values=sorted(self.audn_idx.values()), state="readonly"
        )
        audnCbx.grid(row=2, column=1, sticky="snew", padx=5, pady=2)
        if order.audn_id:
        ...
        mattypeCbx = Combobox(
            orderFrm, values=sorted(self.mattype_idx.values()), state="readonly"
        )
        mattypeCbx.grid(row=3, column=1, sticky="snew", padx=5, pady=2)
        if order.matType_id:
            mattypeCbx.set(self.mattype_idx[order.matType_id])

        Label(orderFrm, text="PO").grid(row=4, column=0, sticky="snw", padx=2, pady=2)
        poEnt = Entry(orderFrm)
        poEnt.grid(row=4, column=1, sticky="snew", padx=5, pady=2)
        if order.poPerLine:
        ...
        noteEnt.grid(row=5, column=1, sticky="snew", padx=5, pady=2)
        if order.note:
        ...
        commentEnt.grid(row=6, column=1, columnspan=3, sticky="snew", padx=5, pady=2)
        if order.comment:
        ...
        oidEnt.grid(row=0, column=3, sticky="snew", padx=5, pady=2)
        if order.oid:
        ...
        bidEnt.grid(row=1, column=3, sticky="snew", padx=5, pady=2)
        if order.bid:
        ...
        wloEnt.grid(row=2, column=3, sticky="snew", padx=5, pady=2)
        if order.wlo:
    ...
    def create_resource_frame(self, parent, resource):
        ...
        resourceFrm.grid(row=0, column=0, columnspan=4, sticky="snew", padx=5, pady=5)
        ...
        if resource.dup_babel:
            babeldupBtn = Button(resourceFrm, image=self.babeldupImg)
            babeldupBtn.grid(row=1, column=0, sticky="nw", padx=2, pady=5)
        ...
        resdataTxt.grid(
            row=0, column=1, rowspan=10, columnspan=3, sticky="snew", padx=10, pady=5
        )
        ...
        editBtn.grid(row=0, column=4, sticky="ne", padx=5, pady=5)
        ...
        deleteBtn.grid(row=0, column=5, sticky="ne", padx=2, pady=5)
    ...
    def display_selected_orders(self, order_ids):
        ...
        for orec in recs:
            ntb = self.order_widget(self.preview_frame, row + 1, orec, cart_pos)
            ntb.grid(row=row, column=0, sticky="snew", padx=2, pady=2)
            row += 1
            cart_pos += 1
    ...
    def distribution_observer(self, *args):
        if self.activeW.get() == "CartView":
            if self.profile.get() != "All users":
                ...
                profile_id = get_id_from_index(self.profile.get(), self.profile_idx)
    ...
    def more_tab_widget(self, tab, resource):
        ...
        urlBtn.grid(row=0, column=0, sticky="sne", padx=20, pady=20)

        scrollbar = Scrollbar(tab, orient=VERTICAL)
        scrollbar.grid(row=1, column=1, rowspan=5, sticky="snw")

        summaryTxt = Text(
            tab, font=RFONT, wrap="word", height=12, yscrollcommand=scrollbar.set
        )
        summaryTxt.grid(row=1, column=0, rowspan=5, sticky="snew", padx=10)
        scrollbar["command"] = summaryTxt.yview

        self.populate_more_tab_summary(summaryTxt, resource.misc, resource.summary)
        ...
        if len(self.order_ids) > 30:
            ...
            self.orders_displayed.set(
                "records {}-{} out of {}".format(
                    self.disp_start + 1, self.disp_end, total_orders
                )
            )
    ...
    def nav_next(self):
        ...
        if self.disp_end < len(self.order_ids):
            ...
            self.selected_order_ids = self.order_ids[self.disp_start : self.disp_end]
            # mlogger.debug(f'Nav next: {self.disp_start}:{self.disp_end}')
            # mlogger.debug(f'Selected order ids: {self.selected_order_ids}')
            self.display_selected_orders(self.selected_order_ids)
            self.orders_displayed.set(
                "records {}-{} out of {}".format(
                    self.disp_start + 1, self.disp_end, len(self.order_ids)
                )
            )
        self.cur_manager.notbusy()
    ...
    def order_widget(self, parent, no, orec, cart_pos):
        ...
        ntb = Notebook(parent, width=1000)
        ...
        gridFrm.grid(row=1, column=1, sticky="snew", padx=10)

        gridCbx = Combobox(
            gridFrm, values=sorted(self.grid_idx.values()), state="readonly"
        )
        gridCbx.grid(row=0, column=0, columnspan=4, sticky="snew", padx=5, pady=2)
        ...
        applyBtn.grid(row=0, column=5, sticky="snw", padx=5, pady=2)
        ...
        copyBtn.grid(row=0, column=6, sticky="snw", padx=5, pady=2)
        ...
        total_qty = 0
        totalQtyLbl = Label(gridFrm, font=LFONT, textvariable=total_qty_lbl)
        totalQtyLbl.grid(row=1, column=4, columnspan=3, sticky="snw", padx=2)
        ...
        Label(gridFrm, text="qty", font=LFONT).grid(row=1, column=2, sticky="sw")
        Label(gridFrm, text="fund", font=LFONT).grid(
            row=1, column=3, sticky="sew", padx=2
        )

        locsFrm = Frame(gridFrm)
        locsFrm.grid(row=2, column=0, columnspan=4, sticky="snew")
        ...
        if orec.locations:
            ...
            for loc in orec.locations:
                ...
                grid_tracker = self.create_grid(
                    locsFrm, r, (loc.did, branch, shelf, qty, fund)
                )
        ...
        # miscellaneous tab
        moreTab = Frame(ntb)
        more_tracker = self.more_tab_widget(moreTab, orec.resource)
        ...
        self.previous_snapshot[ntb.winfo_id()] = dict(
            ord_snapshot=ord_snapshot, grids_snapshot=grids_snapshot
        )
    ...
    def observer(self, *args):
        if self.activeW.get() == "CartView":
            if self.profile.get() != "All users":
                ...
                # funds
                self.fund_idx = create_code_index(Fund, system_id=self.system.get())
                mlogger.debug(
                    "CartView activation funds refresh: {}".format(
                        sorted(self.fund_idx.values())
                    )
                )

                # branches
                self.branch_idx = create_code_index(Branch, system_id=self.system.get())
        ...
        if self.activeW.get() == "CartView":
            if self.profile.get() != "All users":
                ...
                self.distr = get_names(
                    DistSet, system_id=self.system.get(), user_id=profile_id
                )
                self.distCbx["values"] = self.distr

            else:
                msg = "Cart must have an owner.\nPlease select " "one in profile"
    ...
    def remove_location(self, removeBtn):
        ...
        self.tracker[ntb_id]["grid"]["totalQtyLbl"].set(f"total qty:{new_qty}")
    ...
    def save_cart(self):
        try:
            ...
            mlogger.debug(f"Cart needs validation: {needs_validation}")
            ...
            for key, value in self.tracker.items():
                ...
                if self.previous_snapshot[key] != current_snapshot:
                    ...
                    mlogger.debug(f"Order no.: {order_id} changed.")
            ...
            if str(self.cartEnt["state"]) != "disable":
                ...
                save_data(Cart, self.cart_id.get(), **kwargs)
        ...
        try:
            ...
            if self.new_dist.get() != "":
                ...
                if self.new_grid.get() != "":
                    ...
                    messagebox.showerror(
                        "Input error", "Please provide name for the new grid."
                    )
            else:
                if self.dist_template == "new distribution":
                    messagebox.showerror(
                        "Input error", "Please provide name for the new distribution."
                    )
                else:
    ...
    def search_observer(self, *args):
        ...
        offset = pos / (
            (ntc * nt_size) + (ntc * between_gap) + (loc * loc_size) + start_gap
        )
        mlogger.debug(f"CartView: selected record offset: {offset}")
    ...
    def show_fund_widget(self):
        ...
        if str(self.libCbx["state"]) != "disable":
            ...
            Label(frm, text="select funds:").grid(row=0, column=0, sticky="snw", pady=5)
            applyBtn = Button(
                frm,
                image=self.saveImgS,
                command=lambda: self.apply_funds(listbox, listbox.curselection()),
            )
            applyBtn.grid(row=0, column=1, sticky="se", padx=5, pady=10)
            self._createToolTip(applyBtn, "apply selected funds")

            scrollbar = Scrollbar(frm, orient=VERTICAL)
            scrollbar.grid(row=1, column=2, sticky="sne", pady=5)
            listbox = Listbox(
                frm, font=RFONT, selectmode=EXTENDED, yscrollcommand=scrollbar.set
            )
            listbox.grid(row=1, column=0, columnspan=2, sticky="snew", pady=5)
    ...
    def _onValidatePrice(self, i, d, P):
        mlogger.debug(f"_onValidatePrice entered: {P}, index: {i}, action {d}")
        valid = True
        # case 1
