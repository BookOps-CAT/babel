...
class Base(Tk):
    def __init__(self, *args, **kwargs):
        ...
        for name in users:
            profile.menu.add_radiobutton(label=name, variable=self.profile, value=name)

        systemLbl = Label(container, text="System:")
        systemLbl.grid(row=0, column=4, sticky="snw")

        bplBtn = Radiobutton(container, text="BPL", variable=self.system, value=1)
        bplBtn.grid(row=0, column=5, sticky="snw")
        nypBtn = Radiobutton(container, text="NYPL", variable=self.system, value=2)
        ...
        navig_menu.add_command(
            label="Home", command=lambda: self.show_frame("HomeView")
        )
        navig_menu.add_command(
            label="Import", command=lambda: self.show_frame("ImportView")
        )
        navig_menu.add_command(
            label="Carts", command=lambda: self.show_frame("CartsView")
        )
        navig_menu.add_command(
            label="Reports", command=lambda: self.show_frame("ReportWizView")
        )
        navig_menu.add_command(
            label="Grids", command=lambda: self.show_frame("GridView")
        )
        navig_menu.add_command(
            label="Funds", command=lambda: self.show_frame("FundView")
        )
        navig_menu.add_command(
            label="Tables", command=lambda: self.show_frame("TableView")
        )
        navig_menu.add_command(
            label="Settings", command=lambda: self.show_frame("SettingsView")
        )
