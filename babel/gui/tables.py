...
from data.datastore import Audn, Branch, Lang, MatType, ShelfCode, User, Vendor
from errors import BabelError
from gui.data_retriever import get_names, get_record, save_data, delete_data
...
class TableView(Frame):
    ...
    def __init__(self, parent, controller, **app_data):
        ...
        # tables list
        Label(self, text="Tables:").grid(row=0, column=0, sticky="nw")
        ...
        self.genLst.grid(row=1, column=0, rowspan=40, sticky="snew")
        ...
        self.detLbl.grid(row=0, column=3, sticky="nw")
        scrollbarB = Scrollbar(self, orient=VERTICAL)
        scrollbarB.grid(row=1, column=4, rowspan=40, sticky="nsw")
        self.detLst = Listbox(
            self, font=RFONT, height=list_height, yscrollcommand=scrollbarB.set
        )
        self.detLst.bind("<Double-Button-1>", self.set_detail_variable)
        self.detLst.grid(row=1, column=3, rowspan=40, sticky="snew")
        scrollbarB["command"] = self.detLst.yview

        # action buttons
        self.addBtn = Button(self, image=addImg, command=self.add_data)
        self.addBtn.grid(row=1, column=5, sticky="sw", padx=20, pady=10)
        self.createToolTip(self.addBtn, "add new value")

        self.editBtn = Button(self, image=editImg, command=self.edit_data)
        self.editBtn.grid(row=2, column=5, sticky="sw", padx=20, pady=5)
        self.createToolTip(self.editBtn, "edit value")

        self.deleteBtn = Button(self, image=deleteImg, command=self.delete_data)
        self.deleteBtn.grid(row=3, column=5, sticky="sw", padx=20, pady=5)
        self.createToolTip(self.deleteBtn, "delete value")

        self.saveBtn = Button(self, image=saveImg, command=self.insert_or_update_data)
        self.saveBtn.grid(row=4, column=5, sticky="sw", padx=20, pady=5)
        self.createToolTip(self.saveBtn, "save value")

        self.helpBtn = Button(self, image=helpImg, command=self.help)
        self.helpBtn.grid(row=5, column=5, sticky="sw", padx=20, pady=5)
    ...
    def add_data(self):
        ...
        if self.gen_list_select.get():
            if self.gen_list_select.get() in ("Branches", "Shelf Codes"):
                if self.system.get():
    ...
    def delete_data(self):
        ...
        # ask before deletion
        if self.record:
            msg = "Are you sure you want to delete\n{}?".format(str(self.record))
            if messagebox.askokcancel("Deletion", msg):
                mlogger.info("Data for deletion: {}".format(str(self.record)))
    ...
    def insert_or_update_data(self):
        ...
        try:
            if self.gen_list_select.get() in ("Audiences", "Languages"):
            ...
            if self.gen_list_select.get() == "Branches":
                ...
                if self.system.get() == 2:
                    is_research = self.is_research.get()
                else:
                    is_research = None
                if self.temp_closed.get() == 0:
                    temp_closed = False
                else:
                    temp_closed = True
            ...
            elif self.gen_list_select.get() in ("Users", "Vendors"):
                name = self.nameEnt.get().strip()
                bpl_code = self.bplcodeEnt.get().strip()
                nyp_code = self.nypcodeEnt.get().strip()
                kwargs = {"name": name, "bpl_code": bpl_code, "nyp_code": nyp_code}
                ...
                kwargs = {"name": name, "code": code, "includes_audn": includes_audn}
            ...
            mlogger.warning(f"Encounterd TclError: {exc}")
            pass
        ...
        if self.record:
            ...
            try:
                save_data(model, did=self.record.did, **kwargs)
    ...
    def get_corresponding_model(self, **kwargs):
        mlogger.debug("Getting db model for {}".format(self.gen_list_select.get()))
    ...
    def populate_detail_frame(self, *args):
        mlogger.debug(
            "Populating details frame for {}/{}.".format(
        ...
        if self.gen_list_select.get() in ("Audiences", "Languages"):
            if self.det_list_select.get():
                ...
                self.simpleDetailFrame(self.record.name, self.record.code)
            else:
                mlogger.debug("Creating empty details frame.")
                self.simpleDetailFrame()
        elif self.gen_list_select.get() == "Branches":
            if self.det_list_select.get():
                self.record = get_record(model, name=self.det_list_select.get())
                self.branchDetailFrame(
                    self.record.name,
                    self.record.code,
                    self.record.is_research,
                    self.record.temp_closed,
                )
            else:
                mlogger.debug("Creating empty details frame.")
                self.branchDetailFrame()
            ...
            if self.det_list_select.get():
                ...
                self.userDetailFrame(
                    self.record.name, self.record.bpl_code, self.record.nyp_code
                )
            else:
                mlogger.debug("Creating empty details frame.")
                self.userDetailFrame()
            ...
            if self.det_list_select.get():
                ...
                self.mattypeDetailFrame(
                    self.record.name,
                    self.record.bpl_bib_code,
                    self.record.bpl_ord_code,
                    self.record.nyp_bib_code,
                    self.record.nyp_ord_code,
                )
            else:
                mlogger.debug("Creating empty details frame.")
                self.mattypeDetailFrame()
            ...
            if self.det_list_select.get():
                self.record = get_record(
                    model, name=self.det_list_select.get(), system_id=self.system.get()
                )
                self.shelfcodeDetailFrame(
                    self.record.name, self.record.code, self.record.includes_audn
                )
            else:
                mlogger.debug("Creating empty details frame.")
                self.shelfcodeDetailFrame()
    ...
    def userDetailFrame(self, name="", bpl_code="", nyp_code=""):
        ...
        self.nameEnt = Entry(self.detFrm, font=RFONT)
        self.nameEnt.grid(row=0, column=1, columnspan=3, padx=5, pady=5)
        self.bplcodeEnt = Entry(self.detFrm, font=RFONT)
        self.bplcodeEnt.grid(row=1, column=1, columnspan=3, padx=5, pady=5)
        self.nypcodeEnt = Entry(self.detFrm, font=RFONT)
        self.nypcodeEnt.grid(row=2, column=1, columnspan=3, padx=5, pady=5)
        self.nameEnt.insert(0, name)
        self.bplcodeEnt.insert(0, bpl_code)
        self.nypcodeEnt.insert(0, nyp_code)
        disable_widgets([self.nameEnt, self.bplcodeEnt, self.nypcodeEnt])
    ...
    def simpleDetailFrame(self, name="", code=""):
        ...
        self.codeEnt.grid(row=1, column=1, columnspan=3, padx=5, pady=5)
        self.nameEnt.insert(0, name)
        self.codeEnt.insert(0, code)
        disable_widgets([self.nameEnt, self.codeEnt])

    def branchDetailFrame(self, name="", code="", is_research=0, temp_closed=0):
        self.is_research = IntVar()
        self.is_research.set(is_research)
        self.temp_closed = IntVar()
        self.temp_closed.set(temp_closed)

        mlogger.debug("branchDetailFrame activated.")
        if code is None:
            code = ""
        ...
        if self.system.get() == 2:
            Label(self.detFrm, text="is research").grid(
                row=2, column=0, sticky="snw", padx=5, pady=5
            )
            self.isResearchChx = Checkbutton(
                self.detFrm, variable=self.is_research, onvalue=1, offvalue=0
            )
            self.isResearchChx.grid(row=2, column=1, columnspan=3, padx=5, pady=5)

        Label(self.detFrm, text="temp closed").grid(
            row=3, column=0, sticky="snw", padx=5, pady=5
        )
        self.tempClosedChx = Checkbutton(
            self.detFrm, variable=self.temp_closed, onvalue=1, offvalue=0
        )
        self.tempClosedChx.grid(row=3, column=1, columnspan=3, padx=5, pady=5)

        self.nameEnt.insert(0, name)
        self.codeEnt.insert(0, code)
        if self.system.get() == 2:
            disable_widgets(
                [self.nameEnt, self.codeEnt, self.isResearchChx, self.tempClosedChx]
            )
        else:
            disable_widgets([self.nameEnt, self.codeEnt, self.tempClosedChx])
    ...
    def vendorDetailFrame(self, name="", note="", bpl_code="", nyp_code=""):
        ...
        self.noteEnt.grid(row=1, column=1, columnspan=3, padx=5, pady=5)
        self.bplcodeEnt = Entry(
            self.detFrm,
            font=RFONT,
        )
        self.bplcodeEnt.grid(row=2, column=1, columnspan=3, padx=5, pady=5)
        self.nypcodeEnt = Entry(
            self.detFrm,
            font=RFONT,
        )
        self.nypcodeEnt.grid(row=3, column=1, columnspan=3, padx=5, pady=5)
        self.nameEnt.insert(0, name)
        self.noteEnt.insert(0, note)
        self.bplcodeEnt.insert(0, bpl_code)
        self.nypcodeEnt.insert(0, nyp_code)
        disable_widgets([self.nameEnt, self.noteEnt, self.bplcodeEnt, self.nypcodeEnt])
    ...
    def mattypeDetailFrame(
        ...
        self.bplbibcodeEnt.grid(row=1, column=1, columnspan=3, padx=5, pady=5)
        self.bplordcodeEnt = Entry(
            self.detFrm,
            font=RFONT,
        )
        self.bplordcodeEnt.grid(row=2, column=1, columnspan=3, padx=5, pady=5)
        self.nypbibcodeEnt = Entry(
            self.detFrm,
            font=RFONT,
        )
        self.nypbibcodeEnt.grid(row=3, column=1, columnspan=3, padx=5, pady=5)
        self.nypordcodeEnt = Entry(
            self.detFrm,
            font=RFONT,
        )
        self.nypordcodeEnt.grid(row=4, column=1, columnspan=3, padx=5, pady=5)
        self.nameEnt.insert(0, name)
    ...
    def shelfcodeDetailFrame(self, name="", code="", includes_audn=True):
        ...
        self.nameEnt = Entry(self.detFrm, font=RFONT)
        self.nameEnt.grid(row=0, column=1, columnspan=3, padx=5, pady=5)
        self.codeEnt = Entry(self.detFrm, font=RFONT)
        self.codeEnt.grid(row=1, column=1, columnspan=3, padx=5, pady=5)
        self.includesaudnChx = Checkbutton(
            self.detFrm, variable=self.includes_audn, onvalue=1, offvalue=0
        )
        self.includesaudnChx.grid(row=2, column=1, columnspan=3, padx=5, pady=5)
        self.nameEnt.insert(0, name)
        self.codeEnt.insert(0, code)
        if includes_audn:
            self.includes_audn.set(1)
            ...
            self.includes_audn.set(0)
        disable_widgets([self.nameEnt, self.codeEnt, self.includesaudnChx])
    ...
    def system_observer(self, *args):
        if self.activeW.get() == "TableView":
            ...
            if self.gen_list_select.get():
                mlogger.debug("Table {} active".format(self.gen_list_select.get()))
                self.populate_detail_list()
