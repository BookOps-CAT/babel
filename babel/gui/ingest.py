...
class ImportView(Frame):
    ...
    def __init__(self, parent, controller, **app_data):
        ...
        self.vlcl = (self.register(self.onValidateCol), "%d", "%i", "%P", "%W")
        self.vlen = (self.register(self.onValidateSheetName), "%P")
        self.vlcn = (self.register(self.onValidateCartName), "%P")

        # layout
        self.actionFrm = Frame(self)
        self.actionFrm.grid(row=0, column=0, sticky="snew", padx=10, pady=10)

        # action buttons
        self.addBtn = Button(self.actionFrm, image=addImg, command=self.add_data)
        # self.addBtn.image = addImg
        self.addBtn.grid(row=0, column=0, sticky="sw", padx=20, pady=10)
        ...
        self.editBtn.grid(row=1, column=0, sticky="sw", padx=20, pady=5)
        self.createToolTip(self.editBtn, "edit sheet template")

        self.deleteBtn = Button(
            self.actionFrm, image=deleteImg, command=self.delete_data
        )
        # self.deleteBtn.image = deleteImg
        self.deleteBtn.grid(row=2, column=0, sticky="sw", padx=20, pady=5)
        self.createToolTip(self.deleteBtn, "delete sheet template")

        self.saveBtn = Button(
            self.actionFrm, image=saveImg, command=self.insert_or_update_data
        )
        # self.saveBtn.image = saveImg
        self.saveBtn.grid(row=3, column=0, sticky="sw", padx=20, pady=5)
        ...
        self.helpBtn.grid(row=4, column=0, sticky="sw", padx=20, pady=5)
        self.createToolTip(self.helpBtn, "help")

        # sheet template data frame
        self.templateFrm = LabelFrame(self, text="sheet template")
        self.templateFrm.grid(row=0, column=1, sticky="snew", padx=5, pady=10)
        ...
        self.sheetCbx.grid(row=0, column=0, columnspan=2, sticky="snew", padx=5, pady=5)

        self.createdLbl = Label(self.templateFrm, textvariable=self.created, font=LFONT)
        self.createdLbl.grid(
            row=1, column=0, columnspan=2, sticky="snw", padx=5, pady=2
        )
        self.updatedLbl = Label(self.templateFrm, textvariable=self.updated, font=LFONT)
        ...
        self.headerrowEnt = Entry(
            ...
            validate="key",
            validatecommand=self.vlcl,
        )
        self.headerrowEnt.grid(row=3, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.titlecolEnt.grid(row=4, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.addtitlecolEnt.grid(row=5, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.authorcolEnt.grid(row=6, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.seriescolEnt.grid(row=7, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.publishercolEnt.grid(row=8, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.pubdatecolEnt.grid(row=9, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.pubplacecolEnt.grid(row=10, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.summarycolEnt.grid(row=11, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.isbncolEnt.grid(row=12, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.upccolEnt.grid(row=13, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.othernocolEnt.grid(row=14, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.pricelistcolEnt.grid(row=15, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.pricedisccolEnt.grid(row=16, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.urldesccolEnt.grid(row=17, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.commentcolEnt.grid(row=18, column=1, sticky="snw", padx=5, pady=2)
        ...
        self.misccolEnt.grid(row=19, column=1, sticky="snw", padx=5, pady=2)

        Label(self.templateFrm, text="* columns not mapped to MARC").grid(
            row=20, column=0, columnspan=2, sticky="snw", padx=5, pady=5
        )

        # sheet frame
        self.sheetFrm = LabelFrame(self, text="sheet")
        self.sheetFrm.grid(row=0, column=2, sticky="snew", padx=5, pady=10)
        ...
        self.loadBtn.grid(row=0, column=0, sticky="snw", padx=50, pady=20)
        ...
        self.importBtn.grid(row=0, column=0, sticky="sne", padx=50, pady=20)

        # sheet display frame
        self.sheetPreviewFrm = Frame(self.sheetFrm)
        self.sheetPreviewFrm.grid(row=1, column=0, columnspan=3, sticky="snw")
        self.sheetPreviewFrm.columnconfigure(2, minsize=800)

        # sheet preview
        self.xscrollbar = Scrollbar(self.sheetPreviewFrm, orient=HORIZONTAL)
        self.xscrollbar.grid(row=10, column=1, columnspan=7, sticky="nwe", padx=10)
        self.yscrollbar = Scrollbar(self.sheetPreviewFrm, orient=VERTICAL)
        self.yscrollbar.grid(row=0, column=0, sticky="nse", padx=2)
        self.preview_base = Canvas(
            self.sheetPreviewFrm,
            bg="gray",
        ...
        self.preview_base.grid(row=0, column=1, columnspan=7, sticky="nwe", padx=10)
        self.preview()
    ...
    def generate_preview(self):
        ...
        if self.fh:
            ...
            if self.record:
                ...
                mlogger.debug("Applying template mask: {}".format(tmask))
            try:
                sheet_reader = SheetReader(self.fh)
                max_row = sheet_reader.max_row
                max_column = sheet_reader.max_column
                rowLst = Listbox(
                    self.preview_frame, font=LFONT, width=3, height=max_row + 1
                )
                rowLst.grid(row=1, column=1, sticky="nsw")
                ...
                if self.record:
                    rowLst.itemconfig(self.record.header_row + 1, {"bg": "SteelBlue1"})
                c = 0
                r = 0
                for column in range(max_column):
                    colLst = Listbox(
                        self.preview_frame, font=LFONT, width=15, height=max_row + 1
                    )
                    colLst.grid(row=1, column=c + 2, sticky="nsw")
                    if self.record:
                        if column in tmask:
                ...
                messagebox.showerror("Loading sheet error", f"Error: {e}")
            else:
                self.cur_manager.notbusy()
    ...
    def insert_or_update_data(self, *args):
        if self.sheet_name.get() and self.header_row.get() and self.edit_mode:
            kwargs = {
            ...
            kwargs["user_id"] = get_id_from_index(self.profile.get(), self.profile_idx)
            kwargs["updated"] = datetime.now()
            try:
                if self.record:
                    mlogger.debug(
                        "Updating sheet template for record.did: {} "
                        "with values {}".format(self.record.did, kwargs)
                    )
                    save_data(Sheet, did=self.record.did, **kwargs)
                else:
                    mlogger.debug(
                        "Saving new sheet template with values: {}".format(kwargs)
                    )
                    save_data(Sheet, **kwargs)
                    self.created.set(f'created:{kwargs["updated"]:%y-%m-%d %H:%M}')

                self.updated.set(f'updated:{kwargs["updated"]:%y-%m-%d %H:%M}')
    ...
    def open_sheet(self):
        ...
        if fh:
            ...
            if fh.rfind(".xlsx") == -1:
                msg = (
                    "Wrong type of spreadsheet file.\n"
    ...
    def name_cart_widget(self):
        if self.fh and self.record:
            ...
            try:
                c = 0
                data = create_resource_reader(self.record, self.fh)
                for d in data:
                    c += 1
            except BabelError:
                raise
                ...
                mlogger.error(
                    "Unhandled error on create_resource_reader." f"Traceback: {tb}"
                )
            ...
            nameEnt.grid(row=1, column=0, columnspan=2, sticky="snew")
            Label(frm, text=system).grid(row=2, column=0, sticky="snew", pady=5)
            Label(frm, text=profile).grid(row=3, column=0, sticky="snew", pady=5)
            ...
            progbar.grid(row=3, column=0, columnspan=2, sticky="snew", pady=10)
            progbar["maximum"] = c + 1
            ...
            okBtn = Button(
                ...
                command=lambda: self.import_sheet(
                    top, progbar, cart_name.get(), self.system.get(), profile_id
                ),
            )
            okBtn.grid(row=4, column=0, sticky="snew", padx=25, pady=10)
            cancelBtn = Button(frm, text="cancel", command=top.destroy)
            cancelBtn.grid(row=4, column=1, sticky="snew", padx=25, pady=10)

            top.wait_window()

    def import_sheet(self, top_widget, progbar, cart_name, system_id, profile_id):
        if cart_name:
            data = create_resource_reader(self.record, self.fh)
            try:
                cart_id = create_cart(cart_name, system_id, profile_id, data, progbar)
