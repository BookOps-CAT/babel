...
class Vendor(Base):
    ...
    bplCode = Column(String(5), nullable=False, unique=True)
    nyplCode = Column(String(5), nullable=False, unique=True)
...
class DistrCode(Base):
    ...
    distLocQtys = relationship("DistrLocQuantity",
                               cascade='all, delete-orphan')
...
class OrderSingle(Base):
    ...
    oNumber = Column(String)  # add exact length of Sierra order number
    bNumber = Column(String)  # add exact length of Sierra bib number
...
def db_path():
    ...
    if 'local_db_dir' in temp_data:
        dir = temp_data['local_db_dir']
    else:
        dir = ''
    base = 'sqlite:///'
    db_name = 'localstore.db'
    fh = base + dir + db_name
    temp_data.close()
    # print fh
    return fh
